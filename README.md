...
- получение списка задач с пагинацией, фильтрацией по статусу, приоритету и
  просроченности, а также сортировкой;
...
```bash
curl 'http://localhost:5050/api/v1/tasks?page=1&per_page=20&done=false&priority=high&sort=due_at&order=asc'
```

Получить только просроченные задачи:

```bash
curl 'http://localhost:5050/api/v1/tasks?overdue=true'
```

Просроченной считается незавершённая задача, дедлайн которой уже наступил.

По умолчанию задачи сортируются по дате создания от новых к старым. Для
сортировки доступны поля `created_at`, `updated_at`, `title`, `priority` и
`due_at`, а направление задаётся значением `asc` или `desc`. Приоритеты
сортируются в порядке `low`, `medium`, `high`. При сортировке по дедлайну задачи
без дедлайна всегда располагаются в конце списка.
...
- `overdue` — `true` возвращает незавершённые задачи с прошедшим дедлайном,
  `false` — задачи, которые сейчас не просрочены;
- `sort` — поле сортировки: `created_at`, `updated_at`, `title`, `priority` или
  `due_at`, по умолчанию `created_at`;
...
каталога `migrations`. Он включает тесты repository и E2E-сценарии, проходящие
через HTTP, service и PostgreSQL. Обычный `make test` не запускает контейнеры. В
GitHub Actions интеграционные тесты выполняются отдельным PostgreSQL job
параллельно с Go-проверками и сборкой Docker-образа.
